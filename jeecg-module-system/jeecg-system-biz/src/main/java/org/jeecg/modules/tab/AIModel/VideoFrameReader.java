 ...
 */
@Slf4j
public class VideoFrameReader implements Runnable {
    ...
    public VideoFrameReader(String videoStreamURL, String weightUrl, String cfgUrl, String namesUrl, RedisUtil redisUtil, WebSocket webSocket, String userId, int number, RedisTemplate redisTemplate) {
    ...
    public void run() {
        ...
        while (capture.read(frame)) {
            ...
            Integer AITime= (Integer) redisTemplate.opsForValue().get(videoStreamURL+"time"+userId);
            long timestamp2=0;
            if(AITime==null){
                log.error("当前未存储时间节点 当前线程数{}",number);
                long timestamp = (long) capture.get(Videoio.CAP_PROP_POS_MSEC);
                redisTemplate.opsForValue().setIfAbsent(videoStreamURL+"time"+userId,timestamp,365, TimeUnit.DAYS);
                continue;
            }else{
               ...
               if(AITime<timestamp){
                     ...
                     redisTemplate.delete(videoStreamURL+"time"+userId);
                     redisTemplate.opsForValue().setIfAbsent(videoStreamURL+"time"+userId,timestamp,365, TimeUnit.DAYS);
                   //更新为最新时间戳
