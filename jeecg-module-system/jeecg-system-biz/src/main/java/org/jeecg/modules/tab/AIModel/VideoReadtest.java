 ...
 */
@Slf4j
public class VideoReadtest implements Runnable {

    private RedisTemplate redisTemplate;
    public String videoUrl;

    public String userId;
    public VideoReadtest(String videoUrl, RedisTemplate redisTemplate, String userId){
        ...
        while (capture.read(frame)) {
            ...
            if(array.length>0){
                ...
                if(mapTime.times<timestamp){
                    ...
                    try {
                        log.warn("【跳跃最新帧】-存放时间戳{}--{}",timestamp,VideoFrameReader.millisecondsToHours(timestamp));
                ...
                log.warn("【跳跃最新帧】-"+StartTime+"存放时间戳{}--{}",timestamp,VideoFrameReader.millisecondsToHours(timestamp));
                mapTime.setTimes((int) timestamp);
                mapTime.setMat(frame);
                try {
                    VideoSendReadCfg.matlist.offer(mapTime);
            ...
            log.warn("【读取最新帧】-当前时间戳{}--{}当前线程数11111:{}",timestamp,VideoFrameReader.millisecondsToHours(timestamp));
        }
        capture.release();
    }
}
